...
- **Generating a default constructor which supports `Validator` and therefore `Guards`** . It even creates some standard validation, see [Underlying types](#Underlying-types). This can be turned of by setting attribute parameter `generateDefaultConstructor` to `false`.
