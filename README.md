...
Behaviour:
- Creating a name with value `null` will throw a `ValidationException<NotNullGuard<Name>>` with message *"Required data 'value' for 'Name' is missing."*.
- Creating a name with value 'ThisNameIsTooLong' will throw a `ValidationException<LengthInRangeGuard>` with message *"Length is out of range for 'Name' 'ThisNameIsTooLong' (Lower bound: '1') (Upper bound: '15').'*.
- Creating a name with value '1337' will throw a `ValidationException<RegexGuard>` with message *"'Name' '1337' is incorrect."*.
...
- Forcing you to **think about the ToString-implementation**. You can implement `ToString`, or letting you choose the default `ToDisplayString`-method. This can be configured using attribute parameter `generateToString`. See also [ToDisplayString](#ToDisplayString).
- Implementing **`Equals`, `GetHashCode`, `CompareTo`, and comparison operators.** This can be turned off by setting attribute parameter `generateComparison` to `false`.
- **Generating a default constructor which supports `Validator` and therefore `Guards`** . It even creates some standard validation, see [Underlying types](#UnderlyingTypes). This can be turned of by setting attribute parameter `generateDefaultConstructor` to `false`.
- The generates chooses to **forbid the usage of a parameterless constructor** by default, by placing an `Obsolete` attribute on a generated parameterless constructor. As structs automatically contain a parameterless constructor. Something that can be forgotten easily. If the constructor is still being called, it will throw an `InvalidOperationException`. This can be turned off by setting `forbidParameterlessConstruction` to false.
- Generating a **static property containing a default instance of the value object**. Value objects are immutable and therefore easily can provided a default value (if needed). This functionality can be turned on by setting `generateStaticDefault` to `true`.
...
- Letting you **choose the property name** which holds the underlying value, by using setting `propertyName`.
- Setting the **accessibility of the property** by default to `private`. This can be changed by setting `propertyIsPublic` to true.
- Letting you **think about if a `ValidationException` or a `SystemException` should be thrown** when a `Guard` invalidates. Using the parameter `useValidationExceptions`.
...
> The property should be set to `private` and non-private expression bodied properties should be created that expose each value of the tuple. See the example below. 
> 

## SimpleValueObjectExample
...
- A `Guid`.
- A `Uuid` (which is included in this package, see [SimpleValueObjectExample](#SimpleValueObjectExample)): a 32-digit UUID without hyphens.
...
- Use the [Identity generator](#IdentityGenerator). This way you can create a readonly struct (which probably lives on the stack).
...
> To enable correct JSON serialization of identities, add the following line to your Program.cs `builder.Services.AddIdentityJsonSerialization();`
...
For example, to only show the ID of an entity, use: ```public string override ToString() => this.ToDisplayString(new { Id = this.Id });```
...
> To enable correct JSON serialization of value tuples, add the following line to Program.cs `builder.Services.AddValueTupleJsonSerialization();'
